...
MONA_LISA_WIDTH = 68
MONA_LISA_HEIGHT = 100

b = TBMP(MONA_LISA_WIDTH, MONA_LISA_HEIGHT)

inBinary = bin(int(monaLisaData, 16))

# Remove '0b' from the start of the hex number:
inBinary = inBinary[2:]

# Add leading zeros to the binary number, if needed:
inBinary = inBinary.rjust(MONA_LISA_WIDTH * MONA_LISA_HEIGHT, '0')

for y in range(MONA_LISA_HEIGHT):
    for x in range(MONA_LISA_WIDTH):
        if inBinary[y * MONA_LISA_WIDTH + x] == '0': # (!) Try switching this to '1'.
            b[x, y] = True
